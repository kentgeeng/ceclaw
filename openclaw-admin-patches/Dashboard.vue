...
async function refreshDashboard() {
  ...
  try {
    ...
    stats.value = {
      ...
      pendingKnowledge: stats.value.pendingKnowledge,
    }

    try {
      const kRes = await fetch('http://172.25.0.12:9000/api/knowledge-pending')
      const kData = await kRes.json()
      stats.value.pendingKnowledge = kData.count ?? 0
    } catch { stats.value.pendingKnowledge = -1 }
  ...
  <NSpin :show="loading">
    <div class="dashboard-page">
      ...
      <NGrid cols="1 s:2 m:3 l:6" responsive="screen" :x-gap="12" :y-gap="12">
        ...
        <NGridItem style="cursor:pointer" @click="$router.push('/knowledge')">
          <StatCard :title="t('pages.dashboard.stats.pendingKnowledge')" :value="stats.pendingKnowledge" :icon="LibraryOutline" color="#f0a500" />
        </NGridItem>
      </NGrid>
