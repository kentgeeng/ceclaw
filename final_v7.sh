#!/bin/bash
# CeLaw Agent v6 全功能迴圈驗證 (v7 - 最終修復完整版)
# 修復：Test 判斷規則加入繁體中文 "全過" 及 "通過"
# 新增：補齊 Sessions 與 Resume 測試

AGENT="$HOME/ceclaw/claw-agent-v6.py"
LOG_DIR="$HOME/ceclaw/test_logs/v7_complete"
REPORT_DIR="$HOME/ceclaw/test_reports"
mkdir -p "$LOG_DIR" "$REPORT_DIR"

REPORT="$REPORT_DIR/report_v7_$(date +%Y%m%d_%H%M%S).txt"

echo "=========================================================="
echo "  CeLaw Agent v6 全功能迴圈驗證 (v7 - 完整版)"
...
for round in $(seq 1 $MAX_ROUNDS); do
  ...
  python3 "$AGENT" --sessions > "$LOG_DIR/S_${round}.log" 2>&1
  ...
  python3 "$AGENT" --resume "W${round}" --no-ws --steps 3 "確認上次任務狀態並繼續" > "$LOG_DIR/R_${round}.log" 2>&1
  if grep -qiE "session|resume|found|繼續|恢復|狀態" "$LOG_DIR/R_${round}.log"; then echo "✅ PASS"; TOTAL_PASS=$((TOTAL_PASS + 1))
  else echo "❌ FAIL"; fi
